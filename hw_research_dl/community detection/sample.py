...
class AliasSampling:
    """
    alias采样方法，关于该算法可参考：https://www.cnblogs.com/Lee-yl/p/12749070.html
    时间复杂度O(1)的采样算法
    """
...
class RandomWalkSampler:
    """
    实现随机游走的方法，此处可以优化为node2vec的采样方式，考虑变边的权重值
    """
...
# graphSage采样
def sample_neighs(adj_lists, sample_num):
    node_num = len(adj_lists)
    # 首先选取节点的所有邻居
    neighs_all = []
    node: int
    for node in range(node_num):
        # 将自己那个节点也加入，避免后续复杂的concat操作
        neighs_all.append([neighs for neighs in range(node_num) if adj_lists[node][neighs] > 0 or neighs == node])
        # 如果过长，进行随机选取
        if len(neighs_all[node]) > sample_num:
            neighs_all[node] = np.random.choice(a=neighs_all[node], size=sample_num)
        # 如果过短要用重复元素补全，相当于有的邻居取很多次
        else:
            cur_len = len(neighs_all[node])
            for i in range(cur_len, sample_num):
                neighs_all[node].append(neighs_all[node][-1])
    return np.array(neighs_all, dtype=np.int32)